...
export default function BodyGraphHDKit({
  ...
  useEffect(() => {
    const handleLoad = () => {
      ...
      // Create gradients for centers - using rust & gold theme
      const centers = ['Head', 'Ajna', 'Throat', 'Spleen', 'Ego', 'G', 'SolarPlexus', 'Sacral', 'Root'];
      const colors = ['#B57C3C', '#D4A574', '#7A2F00', '#7A2F00', '#B57C3C', '#D4A574', '#7A2F00', '#7A2F00', '#B57C3C'];
      ...
      centers.forEach((center, index) => {
        ...
        const slightlyDarkerColor = darkenColor(colors[index], 0.3);
      ...
      // Create gradients for split gates (both personality and design)
      const createGradient = (id: string, x1: string, y1: string, x2: string, y2: string) => {
        ...
        stop1.setAttribute('style', 'stop-color: #7A2F00; stop-opacity: 1'); // Rust for design

        const stop2 = svgDoc.createElementNS(SVG_NS, 'stop');
        stop2.setAttribute('offset', '50%');
        stop2.setAttribute('style', 'stop-color: #1A1A1A; stop-opacity: 1'); // Black for personality
      ...
      activations.forEach((activation) => {
        ...
        if (hangingGate && gateText && gateBackground) {
          ...
          if (activationsToShow === 'all' && personalityGates[gate] && designGates[gate]) {
            ...
            color = '#1A1A1A'; // Black for personality (conscious)
          } else {
            color = '#7A2F00'; // Rust for design (unconscious)
          }
      ...
      if (
        ...
        if (gateSpan) gateSpan.style.fill = '#1A1A1A';
        if (gateConnect10) gateConnect10.style.fill = '#1A1A1A';
        if (gateConnect34) gateConnect34.style.fill = '#1A1A1A';
      }
      ...
      if (
        ...
        if (gateSpan) gateSpan.style.fill = '#7A2F00';
        if (gateConnect10) gateConnect10.style.fill = '#7A2F00';
        if (gateConnect34) gateConnect34.style.fill = '#7A2F00';
      }
    };
